...
async def create_food(createFood: food_schemas.CreateFood, db: Session = Depends(get_db)):
...
async def update_food(food_id: int, updateFood: food_schemas.UpdateFood, db: Session = Depends(get_db)):
